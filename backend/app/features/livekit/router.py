...
async def get_livekit_token(
    ...
    session_id: str = Query(
        "", description="Pre-created session ID for agent to update"
    ),
):
    try:
        metadata = await service.build_room_metadata(
            current_user.id, persona_id, context
        )
        if session_id:
        ...
        jwt_token = service.generate_token(
            str(current_user.id), current_user.full_name, room
        )

        return TokenResponse(token=jwt_token, room=room)
    except Exception as e:
        logger.error(f"Token generation failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")


@router.get("/onboarding-token", response_model=TokenResponse)
async def get_onboarding_token(
    ...
    try:
        ...
        room_name, metadata = service.build_onboarding_metadata(
        ...
        jwt_token = service.generate_token(
            str(current_user.id), current_user.full_name, room_name
        )

        return TokenResponse(token=jwt_token, room=room_name)
    except Exception as e:
