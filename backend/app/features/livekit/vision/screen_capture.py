...
async def stream_screen_to_gemini(room: rtc.Room, gemini_session, fps: float = 0.3):
        ...
        try:
            async for frame_event in stream:
                ...
                realtime_input = types.LiveClientRealtimeInput(
                    media_chunks=[
                        types.Blob(mime_type="image/jpeg", data=buf.getvalue())
                    ]
                )

                # Send it into the active internal session tunnel
                gemini_session._send_client_event(realtime_input)
    ...
    def on_track_subscribed(
        ...
        if publication.source == rtc.TrackSource.SOURCE_SCREENSHARE and isinstance(
