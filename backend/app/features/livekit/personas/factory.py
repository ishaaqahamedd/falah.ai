...
def get_persona_prompt(persona_id: str, crm_context: str = "") -> tuple[str, str]:
    ...
    if persona_id in INVESTORS:
        ...
        config = SALES_CLIENTS[persona_id]  # type: ignore[assignment]
        prompt = build_sales_client_prompt(config, crm_context)  # type: ignore[arg-type]
        return prompt, config.voice

    # Fallback to investor 1
    logger.warning(
        f"Unknown persona ID '{persona_id}', falling back to default investor."
    )
