...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("ACTIVE", "COMPLETED", "CRASHED", name="sessionstatus"),
            nullable=False,
        ),
    ...
    op.create_index(
        op.f("ix_pitch_sessions_user_id"), "pitch_sessions", ["user_id"], unique=False
    )
    op.create_index(
        op.f("ix_pitch_sessions_persona_id"),
        "pitch_sessions",
        ["persona_id"],
        unique=False,
    )
